...
(async function() {
  ...
  const PLUGIN_STATE_KEY = '__pkmBattlePluginState';
  const existingPluginState = window[PLUGIN_STATE_KEY];
  if (existingPluginState && typeof existingPluginState.destroy === 'function') {
    console.log(`${PLUGIN_NAME} 检测到旧实例，准备先卸载再重载...`);
    try {
      await existingPluginState.destroy('reload');
    } catch (e) {
      console.warn(`${PLUGIN_NAME} 卸载旧实例失败:`, e);
    }
  }
  const pluginState = {
    destroy: null,
...
═══════════════════════════════════════════════════════════════
  ...
  async function destroyPlugin(reason = 'reload') {
    console.log(`${PLUGIN_NAME} 开始卸载实例: ${reason}`);

    try {
      if (typeof eventRemoveListener !== 'undefined') {
        if (pluginState.handlers.onChatChanged) eventRemoveListener('CHAT_CHANGED', pluginState.handlers.onChatChanged);
        if (pluginState.handlers.onMessageSwiped) eventRemoveListener('tavern_events.MESSAGE_SWIPED', pluginState.handlers.onMessageSwiped);
        if (pluginState.handlers.onMessageEdited) eventRemoveListener('tavern_events.MESSAGE_EDITED', pluginState.handlers.onMessageEdited);
        if (pluginState.handlers.onGenerationAfterCommands) eventRemoveListener('GENERATION_AFTER_COMMANDS', pluginState.handlers.onGenerationAfterCommands);
        if (pluginState.handlers.onEraWriteDone) eventRemoveListener('era:writeDone', pluginState.handlers.onEraWriteDone);
      }
    } catch (e) {
      console.warn(`${PLUGIN_NAME} 解绑事件失败:`, e);
    }

    if (pluginState.apiReadyTimer) {
      clearTimeout(pluginState.apiReadyTimer);
      pluginState.apiReadyTimer = null;
    }

    try {
      if (typeof uninjectPrompts === 'function') {
        uninjectPrompts([PKM_INJECT_ID, NPC_INJECT_ID, TIME_INJECT_ID, 'pkm_unlock_events']);
      }
    } catch (e) {
      // 忽略清理失败
    }
    ...
    if (window.PKMPlugin && window.PKMPlugin.__state === pluginState) {
      delete window.PKMPlugin;
    }

    if (pluginState.eventEmitWrapped && window.eventEmit === pluginState.wrappedEventEmit && pluginState.originalEventEmit) {
      window.eventEmit = pluginState.originalEventEmit;
    }

    if (window[PLUGIN_STATE_KEY] === pluginState) {
      delete window[PLUGIN_STATE_KEY];
    }
  ...
  while (typeof eventEmit === 'undefined' && retries < 30) {
    await new Promise(resolve => {
      pluginState.apiReadyTimer = setTimeout(resolve, 100);
    });
    pluginState.apiReadyTimer = null;
    retries++;
  }
  ...
  const onChatChanged = () => resetState('切换对话');
  const onMessageSwiped = () => resetState('消息重骰');
  const onMessageEdited = () => resetState('消息编辑');
  const onGenerationAfterCommands = async (detail) => {
    // 0. 清除上次的解锁事件注入（如果有）
  ...
  const onEraWriteDone = async (detail) => {
  ...
  eventOn('tavern_events.MESSAGE_EDITED', onMessageEdited);
  ...
  eventOn('era:writeDone', onEraWriteDone);
  ...
  const currentEventEmit = window.eventEmit;
  if (currentEventEmit && currentEventEmit.__pkmBattleWrapped) {
    pluginState.originalEventEmit = currentEventEmit.__pkmBattleOriginal || currentEventEmit;
    pluginState.wrappedEventEmit = currentEventEmit;
    console.log(`${PLUGIN_NAME} ERA 变量更新拦截器已存在，跳过重复安装`);
  } else if (currentEventEmit) {
    pluginState.originalEventEmit = currentEventEmit;
    ...
    wrappedEventEmit.__pkmBattleWrapped = true;
    wrappedEventEmit.__pkmBattleOriginal = currentEventEmit;
    pluginState.wrappedEventEmit = wrappedEventEmit;
    pluginState.eventEmitWrapped = true;
    window.eventEmit = wrappedEventEmit;
